            ...
            if (KEY_SHORT == key_op.action) {
                ...
                case DOWN_KEY: {
                    if ((SENSOR_PARAM_PAGE_1 <= oled_par.current_index &&
                        ...
                        (OLED_PARAM_PAGE_1 <= oled_par.current_index && oled_par.current_index <= OLED_PARAM_PAGE_2)) {
                        oled_par.add_flag = 1;
                    } else {
...
// printf("key_thread1 %s\n", CONFIG_BOARD_TARGET);
    ...
    for (;;) {
        // printf("key_thread2 %s\n", CONFIG_BOARD_TARGET);
        key_status();
    }
}

K_THREAD_DEFINE(key_tid, 2048, key_thread_entry, NULL, NULL, NULL, 7, 0, 0);
