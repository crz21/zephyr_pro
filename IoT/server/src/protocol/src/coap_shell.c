...
static int coap_cmd_bmp280_get(const struct shell* sh, size_t argc, char** argv)
{
    int bmp280_id = 0;
    int bmp280_buf[10] = {0};
    const char* addr;
    ...
    bmp280_id = shell_strtol(argv[1], 10, &err);
    if (err) {
        shell_error(sh, "Failed to get a valid button id: %s", argv[1]);
        return err;
    }
    ...
    err = coap_bmp280_get_buf(addr, bmp280_id, bmp280_buf);
    if (err) {
        return err;
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(coap_led_subcmd, SHELL_CMD_ARG(set, NULL, coap_cmd_led_set_help, coap_cmd_led_set, 3, 1),
...
SHELL_STATIC_SUBCMD_SET_CREATE(coap_btn_subcmd, SHELL_CMD_ARG(get, NULL, coap_cmd_btn_get_help, coap_cmd_btn_get, 2, 1),
                               SHELL_SUBCMD_SET_END);

static const char coap_cmd_bmp280_get_help[] =
    "Get a button state using CoAP\n"
    "btn get <btn_id> [addr]\n"
    "\tbtn_id: a number defining the button to get from CoAP server\n"
    "\taddr: the IPv6 address of CoAP server. If not defined, it "
...
SHELL_STATIC_SUBCMD_SET_CREATE(coap_bmp280_subcmd,
                               SHELL_CMD_ARG(get, NULL, coap_cmd_bmp280_get_help, coap_cmd_bmp280_get, 2, 1),
                               SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(coap_subcmd, SHELL_CMD(led, &coap_led_subcmd, "Manage a LED using CoAP", NULL),
                               SHELL_CMD(btn, &coap_btn_subcmd, "Manage a button using CoAP", NULL),
                               SHELL_CMD(bmp280, &coap_bmp280_subcmd, "Get bmp280 data using CoAP", NULL),
                               SHELL_SUBCMD_SET_END);
